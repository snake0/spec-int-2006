...
#  endif
		...
		sv_catpv(PL_Sv, "; \
$\"=\"\\n    \"; \
@env = map { \"$_=\\\"$ENV{$_}\\\"\" } sort grep {/^(SPEC|LD_LIBRARY|DYLD_LIBRARY)/} keys %ENV; ");
#ifdef __CYGWIN__
		sv_catpv(PL_Sv,"\
...
#if 0
    else /* Don't do PERL5OPT if SPECPERLOPT is defined */
    if (
#ifndef SECURE_INTERNAL_GETENV
        !PL_tainting &&
#endif
...
#if !defined(DGUX)
	PerlIO_printf(PerlIO_stdout(),
		      Perl_form(aTHX_ "\nThis is specperl, v%"VDf" built for %s",
...
S_init_perllib(pTHX)
{
    ...
    if (!PL_tainting) {
#ifndef VMS
 	s = PerlEnv_getenv("SPECPERLLIB");
        if (s) {
	    incpush(s, TRUE, TRUE, TRUE);
        }
#if 0
        else {  /* Don't do PERL5LIB or PERLLIB */
	...
	int idx = 0;
	    ...
	    do { incpush(buf,TRUE,TRUE,TRUE); } while (my_trnlnm("SPECPERLLIB",buf,++idx));
        }
#if 0
        else /* Don't do PERL5LIB or PERLLIB if SPECPERLLIB is set */
