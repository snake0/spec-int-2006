...
static const char helpstring[] =
    ...
    "\n\nReport bugs to eedelman@acclab.helsinki.fi\n";
...
static const char license[]=
    "\nCopyright (C) 2000--2002 Erik Edelmann <eedelman@acclab.helsinki.fi>\n"
    "Modifications for SPEC benchmarks Copyright (C) 2004 Standard Performance\n"
...
{
    int i, indent, itemindent, target_printed;
    List *files = NULL;
    char *s, *srcfile;
    Dependency *dep;
    ...
    for (i = 1; i < argc; i++) {
        if (strcmp (argv[i], "-h") == 0 || strcmp(argv[i], "--help") == 0) {
            ...
            if (strlen(argv[i]) == 2) {
                ...
                eq = strchr (&argv[i][2], '=');
                if (eq != NULL)
                    s = xstrndup (&argv[i][2], argv[i] - eq - 2);
                else
                    s = xstrdup (&argv[i][2]);
	    ...
	    if (options.obj_dir[strlen (options.obj_dir) - 1] != '/')
	      ...
	      tmp = replace_suffix (srcfile, ".o");
            ...
            if (options.coco) {
                ...
                if (stat(setfile, &blah) == 0) {
                    if (!list_find (dep->includes, setfile, 
                                    COMP_FUN(&strcasecmp)))
        ...
        for (h2 = dep->modules; h2; h2 = h2->next) {
            ...
            if (!(l = list_find (modlist, s, &modstrcmp))) {
                ...
                if (strcasecmp (mod->sourcefile, dep->sourcefile) == 0) {
                    ...
                    printf ("%s ", set_path(mod->modfile_name, options.obj_dir));
            }
