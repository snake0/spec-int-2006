...
#ifdef WINDOWSNT
    /*
     * Some characters in filenames are not legal in DOS/WIN32.  Find
     * those and replace them with '_' and warn user.  We will correct
     * link names, but we will not complain since the file we are linking
     * to will already be corrected.
     */
    {
       char *p;
       int change_flag = 0;

       for(p=current_file_name;*p;p++)
       {
	 if (*p == '\\' ||
	     *p == ':' ||
	     *p == '*' ||
	     ...
	     *p == '|')
	 {
	   *p = '_';
       ...
       if (change_flag)
       {
	 msg_perror ("Invalid DOS character(s) in name changed to '_'");
       }
       for(p=current_link_name;*p;p++)
       {
	 if (*p == '\\' ||
	     *p == ':' ||
	     *p == '*' ||
...
{
/* How does one modify the time and permisions of a directory on NT? */
#ifndef WINDOWSNT
  struct utimbuf acc_upd_times;
