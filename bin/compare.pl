...
#  All Rights Reserved
#
# $Id: compare.pl 4660 2006-07-24 19:56:18Z cloyce $
#
use strict;
use Digest::MD5;
use Compress::Bzip2;
...
my $version = '$LastChangedRevision: 4660 $ '; # Make emacs happier
...
sub spec_diff {
    ...
    if ($file1 =~ /\.bz2$/) {
      ...
      my $tmp = <$fh1>;
      $/ = $tmpeol;
      $data1 = Compress::Bzip2::decompress(\$tmp);
    ...
    if ($file2 =~ /\.bz2$/) {
      ...
      my $tmp = <$fh2>;
      $/ = $tmpeol;
      $data2 = Compress::Bzip2::decompress(\$tmp);
    ...
    $line = 0;
    while (1) {
	last if eof($fh1) && eof($fh2); # Files were equal or we saw all errors
