...
if(0)
{
    ...
    my $perl = ($Is_MacOS || $Is_VMS) ? $^X : "specperl";
    my $headmaybe = '';
    my $tailmaybe = '';
    $script = "show-shebang";
    if ($Is_MSWin32) {
...
EOF
    ...
    $_ = ($Is_MacOS || $Is_VMS) ? `$perl $script` : `$perl $script`;
    s/\.exe//i if $Is_Dos or $Is_Cygwin or $Is_os2;
    s{\bminiperl\b}{specperl}; # so that test doesn't fail with miniperl
    s{is specperl}{is $perl}; # for systems where $^X is only a basename
    s{\\}{/}g;
