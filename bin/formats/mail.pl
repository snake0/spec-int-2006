...
#
# $Id: mail.pl 4214 2006-05-14 14:17:37Z cloyce $
...
my $mail_version = '$LastChangedRevision: 4214 $ '; # Make emacs happier
$mail_version =~ s/^\$LastChangedRevision: (\d+) \$ $/$1/;
...
"\n",
"runspec: ".basename($0).' '.join(' ', @{$::global_config->orig_argv})."\n",
"\n",
...
"\n",
	...
	if (!$r->mail->{'compress'}) {
	    ...
	    $top->attach(Encoding => 'Base64',
			 ...
			 Data     => Compress::Bzip2::memBzip(<$ifh>),
