//----------------------------  dof_tools.cc  ---------------------------
//    $Id: dof_tools.cc,v 1.5 2006/01/24 01:06:25 wolf Exp $
//    Version: $Name:  $
//
...
void DoFTools::make_boundary_sparsity_pattern (const DoFHandler<1> &dof_handler,
					       ...
					       SparsityPattern    &sparsity)
{
                                   // msvc doesn't like this part of the code
                                   // -- thus delete it, since we don't call
                                   // it anyway
...
//WB 2004-08-26: the fact that we append <deal_II_dimension> to the function
//name and not <dim> is clearly a hack, but necessary to make the whole thing
//compile with xlC.
  const unsigned int n_parameters_on_fine_grid
    = compute_intergrid_weights_1<deal_II_dimension> (coarse_grid, coarse_component, fine_grid, fine_component,
...
//name and not <dim> is clearly a hack, but necessary to make the whole thing
//compile with xlC.
  compute_intergrid_weights_2<deal_II_dimension> (coarse_grid, coarse_component,
