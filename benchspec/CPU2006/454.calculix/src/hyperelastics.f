...
!
      subroutine hyperelastics(text,textpart,elcon,nelcon,
...
!
     ...
     &  n,key,i,j,k,ityp,iperturb,iend,jcoef(3,14),ncmat_
!
...
!
     ...
     &icable for this material type; card image:'
                  write(*,'(a132)') text
               endif
...
!
     ...
     &             elcon(i,ntmat,nmat)
            ...
            enddo
            read(textpart(iend+1),'(f40.0)',iostat=istat) 
     &                elcon(0,ntmat,nmat)
            if(istat.gt.0) call inputerror(text)
         enddo
      else
