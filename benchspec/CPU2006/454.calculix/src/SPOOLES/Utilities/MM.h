...
/*
   ...
   ----------------------------------------------------------
*/
/* Using the z modifier silences some warnings from at least one  */
/* picky compiler.  But although the z modifier is in c99, some   */
/* compilers might not have it yet -jh/18/feb/2005                */
...
if ( (count) > 0 ) { \
   if ( (ptr = (type *)malloc((unsigned long)((count)*sizeof(type)))) \
        == NULL ) {\
      fprintf(stderr, \
              "\n ALLOCATE failure : bytes %zd, line %d, file %s", \
              (count)*sizeof(type), __LINE__, __FILE__) ; \
      exit(-1) ; } \
   else if ( MEMORY_DEBUG > 0 ) { \
      fprintf(stderr, \
              "\n ALLOCATE : address %p, bytes %zd, line %d, file %s", \
              ptr, (count)*sizeof(type), __LINE__, __FILE__) ; } } \
else if ( (count) == 0 ) { \
   ptr = NULL ; } \
else { \
   fprintf(stderr, \
           "\n ALLOCATE error : bytes %zd, line %d, file %s", \
           (count)*sizeof(type), __LINE__, __FILE__) ; \
   exit(-1) ; }
#endif
