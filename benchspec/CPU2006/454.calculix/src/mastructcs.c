...
void mastructcs(int *nk, int *kon, int *ipkon, char *lakon,
  ...
  for(i=0;i<*ne;++i){
    ...
    indexe=ipkon[i];
    if((strcmp1(&lakon[8*i],"C3D20R")==0)||
       (strcmp1(&lakon[8*i],"C3D20 ")==0)){nope=20;}
    else if ((strcmp1(&lakon[8*i],"C3D8R ")==0)||
             (strcmp1(&lakon[8*i],"C3D8  ")==0)){nope=8;}
    else {nope=10;}
  ...
  *neq=0;
  ...
  for(i=0;i<*ne;++i){
	...
	if((jdof1!=0)&&(jdof2!=0)){
	  ...
	  kdof1=jdof1+*neq;kdof2=jdof2+*neq;
	  ...
	  if(*nmpc>0){
		...
		if(strcmp1(&labmpc[(id1-1)*20],"CYCLIC")==0){
		  ...
		  FORTRAN(nident,(ics,&inode,&ns[3],&id));
		  if(id>0){
		    if(ics[id-1]==inode) icomplex=1;
		  }
		}
		idof2=nactdof[4*inode+nodempc[3*index-2]-4];
		if(idof2!=0){
		  insert(ipointer,&mast1,&irow,&idof1,&idof2,&ifree,&nzs_);
		  kdof1=idof1+*neq;kdof2=idof2+*neq;
		  insert(ipointer,&mast1,&irow,&kdof1,&kdof2,&ifree,&nzs_);
		  if((icomplex==1)&&(idof1!=idof2)){
	  ...
	  if((mpc1==1)&&(mpc2==1)){
		...
		if(strcmp1(&labmpc[(id1-1)*20],"CYCLIC")==0){
		  ...
		  FORTRAN(nident,(ics,&inode1,&ns[3],&id));
		  if(id>0){
		    if(ics[id-1]==inode1) icomplex1=1;
		  }
		}
		...
		while(1){
		  ...
		  if(strcmp1(&labmpc[(id1-1)*20],"CYCLIC")==0){
		    ...
		    FORTRAN(nident,(ics,&inode2,&ns[3],&id));
		    if(id>0){
		      if(ics[id-1]==inode2) icomplex2=1;
		    }		
		  }  
...
!
      if(neq.eq.0) then
         write(*,*) '*ERROR: no degrees of freedom in the model'
         stop
      endif
!
