...
int BuildRenderStatistics(POVMSObjectPtr msg, COUNTER *pstats)
{
   ...
   if(err == kNoErr)
      err = AddStatistic(msg, kPOVAttrib_BoundingQueues, &pstats[totalQueues]);
   if(err == kNoErr)
      err = AddStatistic(msg, kPOVAttrib_BoundingQueueResets, &pstats[totalQueueResets]);
   if(err == kNoErr)
      err = AddStatistic(msg, kPOVAttrib_BoundingQueueResizes, &pstats[totalQueueResizes]);
   if(err == kNoErr)
